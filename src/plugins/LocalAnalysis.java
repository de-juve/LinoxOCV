...
public class LocalAnalysis extends AbstractPlugin {
    ...
    ParameterSlider thresholdSlider = new ParameterSlider("Threshold for different between road and background:", 10, 50, 20);
    ParameterSlider kernelSize = new ParameterSlider("Size of area closing:", 1, 500, 5);
    ...
    public void getParams( ParameterJPanel panel ) {
        ...
        area_size = Math.min(mask_w * mask_h, panel.getValueSlider(kernelSize));
    ...
    private void analyse() {
        ...
        Mask mask = new Mask(mask_w, mask_h, road_w, grey.type(), threshold);
        L:
        for (int y = 0; y < grey.rows(); y += mask_w / 2) {
            if (y + mask_w > grey.rows()) {
                break;
            }
            for (int x = 0; x < grey.cols(); x += mask_h / 2) {
                if (x + mask_h > grey.cols()) {
                    break;
                }
